...
class classifier_model_trainer:
    '''
    It trains a classifier model with the given X_train and y_train.  It can perform k-group cross-validation. The function is used in the RandomForest_classifier_trainer method.
    Input:
    -----------------
    ...
    def __init__(self, X_train, y_train, strategy='random', kfold=3, **kwargs):
        ...
        if kwargs.get('cv_splitter') is not None:
            ...
            if self.strategy == 'random':
                ...
                raise ValueError("GroupKFold is not supported for classification tasks. Use 'random' strategy instead.")
                #self.groups = kwargs.get("groups")
                #if self.groups is None:
                #    raise ValueError("Groups must be provided for GroupKFold strategy.")
...
class regressor_model_trainer:
    '''
    It trains a regressor model. It can take a list of linear sequences and split the data into training and testing sets. It can also perform k-group cross-validation. The function is used in the RandomForest_regressor_trainer method.
    Input:
    -----------------
    X_train: a 2D array of size (N_features, N_samples)
    y_train: a 1D of size N_samples
    ...
    **kwargs: a dictionary. It can have 'cv_splitter' and 'param_dict' keys. It is used to provide the cv_splitter and parameters for the model.
    ...
    def __init__(self, X_train, y_train, strategy='random', kfold=3, **kwargs):
        ...
        if kwargs.get("scoring") is not None:
            ...
            self.scoring = ["neg_mean_squared_error", "r2", "neg_mean_absolute_error"]
        ...
        if kwargs.get("primary_scoring") is not None:
            ...
            self.primary_scoring = 'neg_mean_absolute_error'
        ...
        if kwargs.get('cv_splitter') is not None:
            ...
            if self.strategy == 'random':
                ...
                raise ValueError("GroupKFold is not supported for regression tasks. Use 'random' strategy instead.")
                #self.cv_splitter = sk.model_selection.GroupKFold(n_splits=self.kfold)
                #self.groups = kwargs.get("groups")
            else:
                raise ValueError("Strategy must be either 'random' or 'groupkfold'")
    ...
    def kfold_trainer_using_random_grid_search(self, classifier_model, param_grid, n_jobs=1):
        ...
        y_pred_train=best_model_found.predict(self.X_train)
        print("Best parameters found: ", random_search.best_params_)
        print("Score on training: MAE=", sk.metrics.mean_absolute_error(self.y_train,y_pred_train), "R2=", sk.metrics.r2_score(self.y_train,y_pred_train))

        return best_model_found, random_search_results_df
    
    def XGBoostRegressor_trainer(self, training_mode='default', **kwargs):
        
        '''
        The purpose it to train a XGBoost regressor
        Input:
        **kwargs: a dictionary. It can have 'n_iter' key. It is used to provide the number of iterations for 
        param_grid: a dictionary. It is used to provide the parameters for the model.
        training_mode: a string. Options are 'default', 'CrossVal'. If 'default', it will train the model with default parameters. If 'CrossVal', it will perform cross-validation using RandomizedSearchCV.
        Output:
        best_model_found: a trained XGBoost model
        grid_search_results_df: a pandas dataframe that has the results of RandomizedSearchCV
        '''
        ...
        if kwargs.get('param_grid') is not None:
            ...
            param_grid = {'objective':['reg:absoluteerror'],
                        'learning_rate': stats.uniform(0.01,0.1),
                        'max_depth': [3,4,5,6,7,8,9,10], #3 is default
                        'min_child_weight': [1,3,5],
                        'gamma': stats.uniform(0.0,0.6),
                        'subsample':  stats.uniform(0.3,0.6),
                        'colsample_bytree': stats.uniform(0.5, 0.4),
                        'n_estimators': [30,50,100, 200,300, 500], 'n_jobs':[-1]}
        
        if training_mode == 'default':
            regressor_model.fit(self.X_train, self.y_train)
            y_pred_train = regressor_model.predict(self.X_train)

            print("Score on training: MAE=", sk.metrics.mean_absolute_error(self.y_train,y_pred_train), "R2=", sk.metrics.r2_score(self.y_train,y_pred_train))

            return regressor_model, pd.DataFrame({'y_true': self.y_train, 'y_pred': y_pred_train})
            ...
            best_model_found, random_search_results_df=self.kfold_trainer_using_random_grid_search(regressor_model, param_grid, n_jobs=n_jobs)
            return best_model_found, random_search_results_df
        else:
            raise ValueError("Training mode must be either 'default' or 'CrossVal'")
